...
public class OXOView extends JPanel {
  ...
  protected void paintComponent(Graphics g) {
    ...
    float horiSpacing = (float) (getWidth() - MARGIN * 2) /
    model.getNumberOfColumns();
    float vertSpacing = (float) (getHeight() - MARGIN * 2) /
    ...
    for (
      int colNumber = 0;
      colNumber < model.getNumberOfColumns();
      colNumber++
    ) {
      for (
        int rowNumber = 0;
        rowNumber < model.getNumberOfRows();
        rowNumber++
      ) {
        int xpos = (int) (
          ((float) MARGIN) +
          2 -
          (FONT_SIZE / 2) +
          (horiSpacing * (colNumber + 0.5f))
        );
        int ypos = (int) (
          ((float) MARGIN) +
          (FONT_SIZE / 2) +
          (vertSpacing * (rowNumber + 0.5f))
        );
        OXOPlayer cellOwner = model.getCellOwner(rowNumber, colNumber);
        if (cellOwner != null) g.drawString(
          "" + cellOwner.getPlayingLetter(),
          xpos,
          ypos
        );
      }
    ...
    if (model.getWinner() != null) message =
      "Player " +
      model.getWinner().getPlayingLetter() +
      " is the winner !"; else if (model.isGameDrawn()) message =
      "Stalemate - game is a draw !"; else message =
      "Player " +
      model
        .getPlayerByNumber(model.getCurrentPlayerNumber())
        .getPlayingLetter() +
      "'s turn";
